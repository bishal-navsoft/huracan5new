...
<script language="javascript" type="text/javascript">
	...
	function confirmFun(me)
	{
		...
		if(confirm("Do You Want To Delete?"))
		{
			...
			var  uploadPath = rootpath+'Sqreports/uploadimage/delete/'+encodeURIComponent(imagename);
	...
	function uploadFile(){
		...
		var rootpath = '<?php echo $this->webroot; ?>';
		$('#loaderRes').html('<br /><img src='+rootpath+'/app/webroot/img/ajax-loader.gif>');
		var  uploadPath = rootpath+'Sqreports/uploadimage/upload';
		document.add_report_attachment_form.action =uploadPath;
		document.add_report_attachment_form.target = "imgFrame";
		document.add_report_attachment_form.submit();
	}

	function add_sq_attach()
	{
        ...
        $.ajax({
			type: "POST",
			url: rootpath+"Sqreports/sqattachmentprocess/",
			...
			success: function(res)
			{
				if(res=='fail'){
					...
					document.location='<?php echo $this->webroot; ?>Sqreports/report_sq_attachment_list/<?php echo base64_encode($report_id); ?>';
				}else if(res=='update'){
...
<?PHP echo $this->Form->input('attachment_description', ['type' => 'text', 'id' => 'attachment_description', 'name' => 'attachment_description', 'value' => $description, 'label' => false, 'class' => 'textclass', 'div' => false]); ?><span class="textcmpul" id="description_error"></span>
<div class="clearflds"></div>
<label><?PHP echo __("Upload File"); ?></label>
<div class="upload-here">
      <input type="file" size="24"  name="data[Sqreports][file_upload]" id="BrowserHidden" onchange="getElementById('FileField').value = getElementById('BrowserHidden').value;" />
...
<div class="clearflds"></div>
<?php echo $this->Form->input('hiddenFile', ['type' => 'hidden', 'label' => false, 'id' => 'hiddenFile', 'name' => 'hiddenFile', 'value' => $imagename]); ?>
 <iframe id="imgFrame" name="imgFrame" src="" style="width:100px;height:100px;border:1px solid #D5E278; display:none"></iframe>
<?php echo $this->Form->end(); ?>
<div class="buttonpanel">
<span id="loader" style="font-size: 13px;"></span> <input type="button" name="save" id="save" class="buttonsave" onclick="add_sq_attach();" value="<?php echo $button; ?>" />
</div>
<?php echo $this->Form->end(); ?>
</section>