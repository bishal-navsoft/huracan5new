  ...
  ?>
	...
	if(incident_val!=0){
        var path='<?php echo $this->webroot; ?>';
         	  ...
         	  $.ajax({
			    type: "POST",
			    url: path+"Sqreports/displayincidentdetail/",
			    data:"incidentid="+incident_val+"&report_id=<?php echo $report_id ?>",
   ...
   function immediate_cause_detail(){
		    ...
		    var rootpath='<?php echo $this->webroot; ?>';  
                     $.ajax({
			  type: "POST",
			  url: rootpath+"Sqreports/retrivecause/",
	...
	var spTRID=trid.split(',');
     ...
     function  add_root_cause(rtc,tdid){
		      ...
		      if(root_cause_id==0){
			   ...
			   document.getElementById("lH").innerHTML='<img src="<?php echo $this->webroot; ?>img/ajaxloader.gif" />';
			      $.ajax({
				  type: "POST",
				  url: rootpath+"Sqreports/retriverootcause/",
     ...
     function add_investigation_data_analysis(){
	  ...
	  var rootpath='<?php echo $this->webroot; ?>';
			      $.ajax({
				  type: "POST",
				  url: rootpath+"Sqreports/save_date_analysis/",
				  ...
				  success: function(res)
				  {
				   ...
				   document.location='<?php echo $this->webroot; ?>Sqreports/add_sq_investigation_data_analysis/'+splres[1]+'/'+splres[2]+'/'+splres[3];
    ...
    echo $this->Form->create('add_report_investigation_data_form', array('controller' => 'Sqreports','name'=>"add_report_investigation_data_form", 'id'=>"add_report_investigation_data_form", 'method'=>'post','class'=>'adminform'));
    ...
    <div class="sub_contentwrap">
	  <table width="100%" border="0" cellspacing="1" cellpadding="0" class="reporttable" >
		       ...
		       <td>
				...
				<option value="0">Select One</option>
				   ...
				   for($inc=0;$inc<count($incidentdetail);$inc++){
					 if($incidentdetail[$inc]['SqReportIncident']['damage_category']!=0){
					?>
					      <option value="<?php echo $incidentdetail[$inc]['SqReportIncident']['id']; ?>" <?php if($incidentdetail[$inc]['SqReportIncident']['id']==$edit_incident_id){echo "selected";} ?>>Incident-<?php echo $incidentdetail[$inc]['SqReportIncident']['incident_no'];?></option>
		    ...
		    <tr>
			 ...
			 <td id="loss_content"><?php echo $damage_type; ?></td>
		    </tr>
		    <tr>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="reporttable" id="cause_content" >
		    ...
		    <tr><td align="left" width="22%" valign="middle" class="label">Comments</td>
		   <td><textarea id='comments'  name='comments'><?php echo $comments; ?></textarea></td></tr>
...
<option value="<?php echo $remidialData[$i]['SqRemidial']['remedial_no'].'~'.$remidialData[$i]['SqRemidial']['remidial_summery'].'~'.$remidialData[$i]['SqRemidial']['id']; ?>"><?php echo 'REM-'.$remidialData[$i]['SqRemidial']['remedial_no']; ?></option>
		    <?php } ?>
              </select>
	    </td>
	   ...
	   <td width="10%" align="right" valign="middle" >
	       ...
	       <?php }else{?>
		    &nbsp;
	      <?php  }?>
	        <input type="hidden" id="id_holder" value="<?php echo $id_holder; ?>" />
	   </td>
     ...
     <table width="100%" border="0" cellspacing="1" cellpadding="0" class="reporttable" id="remedial_holder">
	   <?php
	      if(count($remidialList)>0){
			...
			<tr id='<?php echo 'tr'.$remidialList[$i]['SqRemidial']['id']; ?>'>
			      <td width="12%" align="left" valign="middle" ><?php echo 'REM-'.$remidialList[$i]['SqRemidial']['remedial_no']; ?></td>
			      <td width="78%" align="left" valign="middle" ><?php echo  $remidialList[$i]['SqRemidial']['remidial_action']; ?></td>
			      <td width="10%" align="left" valign="middle">
				   <a href="javascript:void(0);" onclick="remove_child('<?php echo 'tr'.$remidialList[$i]['SqRemidial']['id']; ?>');">Remove</a>
			      </td>
			</tr>
