     ...
     switch(type){
	  	case'depart':
			...
			//}
	       	...
	       	/*if(return_depart_date>current_date){
		  		document.getElementById('return_depart_error').innerHTML='Depart base date Should be less than current date';
	    ...
	    break;
	  	...
	  	case'retrun':
			...
			var strtjob_day=document.getElementById('return_depart_date').value ;
	    	var endjob_day=document.getElementById('return_base_date').value ;
	    	var strtjob_time=document.getElementById('return_depart_time').value ;
	    	var endjob_time=document.getElementById('return_base_time').value ;
	   		var current_date='<?php echo date("m-d-Y"); ?>';

	       	/*if(return_base_day>current_date){
			  	document.getElementById('return_base_error').innerHTML='Return base date should be less than current date';
			  	document.getElementById('return_base_date').value='';
			  	return false;
	       	}else{
		  		document.getElementById('return_base_error').innerHTML='';
		          ...
		          $.ajax({
				  	...
				  	success: function(res)
				  	{
					    ...
					    if(res=='wrong'){
					      	...
					      	document.getElementById('hidden_total_value').value='';
					       	document.getElementById('return_base_error').innerHTML='Return Base date always greater than Depart Base date';
					     }else{
					      document.getElementById('total_time').innerHTML=res;
					      document.getElementById('hidden_total_value').value=res;
					     }
	                }
				});
...
function add_job_report_main()
{
	    ...
	    var total_time = jQuery.trim(document.getElementById('hidden_total_value').value);
     ...
     document.getElementById('loader').innerHTML='<img src="<?php echo $this->webroot; ?>img/loader.gif" />';	
			  ...
			  data:"data="+dataStr+"&report_no="+report_no+"&rp_date="+rp_date+"&business_unit="+business_unit+"&client="+client+"&field_location="+field_location+"&country="+country+"&strtjobval="+strtjobval+"&endjobval="+endjobval+"&report_id="+report_id+"&operating_time="+operating_time+"&total_time="+total_time+"&loss_time="+loss_time+"&return_base="+return_base+"&return_depart_val="+return_depart_val,
...
<?php echo $this->Element('left_menu'); ?>
 ...
 <?php
    ...
    echo $this->Form->create(null, [
        'url' => ['controller' => 'Jobs', 'action' => 'addJobMain'],
        'name' => 'add_job_main_form',
        'id' => 'add_job_main_form',
        'method' => 'post',
        'class' => 'adminform'
    ]);
...
<label><?PHP echo __("Total Time:");?></label>
<label id="total_time"><?PHP echo $total_time;?> </label><input type="hidden" id="hidden_total_value"  value="<?PHP echo $hidden_total_value;?>" />
<div class="clearflds"></div>
