...
function confirmFun(me)
    {
	  ...
	  if(confirm("Do You Want To Delete?"))
	  {
	       ...
	       var  uploadPath = rootpath+'Suggestions/uploadimage/delete/'+encodeURIComponent(imagename);
...
function uploadFile()
     {
	  ...
	  var rootpath = '<?php echo $this->webroot;?>';
	  $('#loaderRes').html('<br /><img src='+rootpath+'/app/webroot/img/ajax-loader.gif>');
	  var  uploadPath = rootpath+'Suggestions/uploadimage/upload';
	  document.add_report_attachment_form.action =uploadPath;
	  document.add_report_attachment_form.target = "imgFrame";
	  document.add_report_attachment_form.submit();
     }
...
function add_sg_attach()
{
	    ...
	    document.getElementById('loader').innerHTML='<img src="<?php echo $this->webroot; ?>img/loader.gif" />';	
	       ...
	       $.ajax({
			  type: "POST",
			  url: rootpath+"Suggestions/suggestionsattachmentprocess/",
			...
			if(res=='fail'){
				   ...
				   document.location='<?php echo $this->webroot; ?>Suggestions/report_suggestion_attachment_list/<?php echo base64_encode($report_id); ?>';
			     }else if(res=='update'){
...
<h2><?php echo $heading; ?> &nbsp; <?php echo $report_number; ?></h2>
  <?php
    echo $this->Form->create('Suggestions', array('type'=>'file','id' => 'add_report_attachment_form','name'=>'add_report_attachment_form','class'=>'adminform'));
