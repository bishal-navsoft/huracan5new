							...
							<select id="personal_data" name="personal_data" >
								...
								<?php foreach ($userDetail as $user): ?>
									<?php
										...
										$roleName = $user->role_master->role_name ?? 'N/A';
										$position = $user->position ?? 'N/A';
										$seniority = $user->user_seniority ?? ''; // e.g. 12/04/2025
										$id = $user->id;
									?>
									<option value="<?= h("$fullName~$roleName~$position~$id~$seniority") ?>">
										<?= h($fullName) ?>
									</option>
								<?php endforeach; ?>
							</select>
...
<div class="sub_contentwrap fixreport_table" >
    ...
    <?php foreach ($investnameHolader as $invest) : ?>
        <?php if (!empty($invest)) : ?>
            <tr id="<?= h($invest['id'] ?? '') ?>">
                <td width="30%" align="left" valign="middle">
                    <?= h(ucfirst($invest['first_name'] ?? '') . ' ' . ($invest['last_name'] ?? '')) ?>
                </td>
                <td width="30%" align="left" valign="middle">
                    <?= h($invest['user_seniority'] ?? '') ?>
                </td>
                <td width="30%" align="left" valign="middle">
                    <?= h($invest['role_name'] ?? '') ?>
                </td>
                ...
                <td width="10%" align="left" valign="middle">
                    <a href="javascript:void(0);" onclick="remove_child(<?= (int)($invest['id'] ?? 0) ?>);">Remove</a>
                </td>
            </tr>
        <?php endif; ?>
    <?php endforeach; ?>
