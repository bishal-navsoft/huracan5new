...
var  uploadPath = rootpath+'Audits/uploadimage/delete/'+encodeURIComponent(imagename);
...
var rootpath = '<?php echo $this->webroot;?>';
$('#loaderRes').html('<br /><img src='+rootpath+'/app/webroot/img/ajax-loader.gif>');
var  uploadPath = rootpath+'Audits/uploadimage/upload';
document.add_report_attachment_form.action =uploadPath;
document.add_report_attachment_form.target = "imgFrame";
document.add_report_attachment_form.submit();
}
...
function add_audit_attach()
{
	    ...
	    document.getElementById('loader').innerHTML='<img src="<?php echo $this->webroot; ?>img/loader.gif" />';	
	       ...
	       $.ajax({
			  type: "POST",
			  url: rootpath+"Audits/auditattachmentprocess/",
			...
			if(res=='fail'){
				   ...
				   document.location='<?php echo $this->webroot; ?>Audits/report_audit_attachment_list/<?php echo base64_encode($report_id); ?>';
			     }else if(res=='update'){
...
<div class="upload-here">
      <input type="file" size="24"  name="data[Audits][file_upload]" id="BrowserHidden" onchange="getElementById('FileField').value = getElementById('BrowserHidden').value;" />
...
<span id="loader" style="font-size: 13px;"></span> <input type="button" name="save" id="save" class="buttonsave" onclick="add_audit_attach();" value="<?php echo $button; ?>" />
</div>
<?php echo $this->Form->end(); ?>
</section>