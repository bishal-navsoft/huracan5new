...
var  uploadPath = rootpath+'Jrns/uploadimage/delete/'+encodeURIComponent(imagename);
...
var rootpath = '<?php echo $this->webroot;?>';
$('#loaderRes').html('<br /><img src='+rootpath+'/app/webroot/img/ajax-loader.gif>');
var  uploadPath = rootpath+'Jrns/uploadimage/upload';
document.add_report_attachment_form.action =uploadPath;
document.add_report_attachment_form.target = "imgFrame";
document.add_report_attachment_form.submit();
}
...
function add_jn_attach()
{
	    ...
	    document.getElementById('loader').innerHTML='<img src="<?php echo $this->webroot; ?>img/loader.gif" />';	
	       ...
	       $.ajax({
			  type: "POST",
			  url: rootpath+"Jrns/jnattachmentprocess/",
			...
			if(res=='fail'){
				   ...
				   document.location='<?php echo $this->webroot; ?>Jrns/report_jn_attachment_list/<?php echo base64_encode($report_id); ?>';
			     }else if(res=='update'){
...
<div class="upload-here">
      <input type="file" size="24"  name="data[Jrns][file_upload]" id="BrowserHidden" onchange="getElementById('FileField').value = getElementById('BrowserHidden').value;" />
...
<span id="loader" style="font-size: 13px;"></span> <input type="button" name="save" id="save" class="buttonsave" onclick="add_jn_attach();" value="<?php echo $button; ?>" />
</div>
<?php echo $this->Form->end(); ?>
</section>