                   ...
                   $.ajax({
                              ...
                              success: function(res)
                              {
                                ...
                                if(res=='ok'){
                                       ...
                                       document.location='<?php echo $this->webroot; ?>Jrns/add_sq_link/<?php echo $report_id_val; ?>';
                                 }
	       ...
	       <select id="report_data" name="report_data" >
			   ...
			   if(count($reportDeatil['JnReportMain'])>0){
                                for($i=0;$i<count($reportDeatil['JnReportMain']);$i++){?>
                                       <option value="<?php echo 'jn_'.$reportDeatil['JnReportMain'][$i]['id'][0].'~'.$reportDeatil['JnReportMain'][$i]['type'][0].'~'.$reportDeatil['JnReportMain'][$i]['report_name'][0].'~'.$reportDeatil['JnReportMain'][$i]['summary'][0];?>" ><?php echo $reportDeatil['JnReportMain'][$i]['report_name'][0]; ?></option>
                                <?php }
                          } 
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="0" class="reporttable" id="report_holder">
    ...
    if(count($linkDetail)>0){
       ...
       for($i=0;$i<count($linkDetail);$i++){?>
        ...
        <tr id="<?php echo $linkDetail[$i]['JnLink']['type'].'_'.$linkDetail[$i]['JnLink']['report_id']; ?>">
           <td width="20%" align="left" valign="middle" ><a href="<?php echo $this->webroot.$linkDetail[$i]['JnLink']['controller'].'/'.$linkDetail[$i]['JnLink']['view'].'/'.base64_encode($linkDetail[$i]['JnLink']['report_id']); ?>" target="_blank"><?php echo $linkDetail[$i]['JnLink']['report_num']; ?></a></td>
	   <td width="20%" align="left" valign="middle" ><?php echo $linkDetail[$i]['JnLink']['report_type']; ?></td>
           <td width="50%" align="left" valign="middle" ><?php echo $linkDetail[$i]['JnLink']['summary'];?></td>
           <td width="10%" align="left" valign="middle" >
	        <a href="javascript:void(0);" onclick="remove_child('<?php echo $linkDetail[$i]['JnLink']['type'].'_'.$linkDetail[$i]['JnLink']['report_id']; ?>');">Remove</a>
            </td>
          </tr>
  ...
  <div class="sub_contentwrap fixreport_table" >
    <table width="100%" border="0" cellspacing="1" cellpadding="0" >
	  ...
	  <td colspan="3" width="100%">
	       <input type="button" name="save" id="save" class="buttonsave" onclick="add_jn_link();" value="<?php echo $button; ?>" /><span id="loader" style="font-size: 13px;"></span>
	  </td>
      </tr>
    </table>
 </div>
