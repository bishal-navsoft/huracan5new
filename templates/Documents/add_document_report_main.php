...
function change_type(){
      ...
      document.getElementById('doc_type').innerHTML=sp_val[1];
 }
function add_report_main()
{
	    ...
	    /*if(validation_date!=''){
	        if(validation_date>currentdate){
	    		...
	    		if(revalidation_date!=''){
			      ...
			      if(revalidation_date_obj<validation_date_obj){
				  	...
				  	document.getElementById('revalidation_date_error').innerHTML='Revalidation date always Greater than Validation Date';
				  	return false;
				  	}else{
	    ...
	    var dataStr = $("#add_documentation_main_form").serialize();
        var rootpath='<?php echo $this->webroot ?>';
     	document.getElementById('loader').innerHTML='<img src="<?php echo $this->webroot; ?>img/loader.gif" />';	
              $.ajax({
			  type: "POST",
			  url: rootpath+"Documents/dcreportprocess/",
			  data:"data="+dataStr+"&report_no="+report_no+"&business_unit="+business_unit+"&field_location="+field_location+"&country="+country+"&validate_by="+validate_by+"&report_id="+report_id+"&validation_date="+validation_date+"&revalidation_date="+revalidation_date+"&d_type="+sp_val[0],
			  ...
			  success: function(res)
			  {
				   ...
				   document.getElementById('loader').innerHTML='<font color="green">Document Main Added Successfully</font>';
				   document.location='<?php echo $this->webroot; ?>Documents/add_document_report_main/'+resval[1];
				   
                             }else if(resval[0]=='update'){
				   document.getElementById('loader').innerHTML='<font color="green">Document Main Updates Successfully</font>';
				   document.location='<?php echo $this->webroot; ?>Documents/add_document_report_main/'+resval[1];
				  }
...
<label><?PHP echo __("Type:");?></label>
				   ...
				   <?php for($i=0;$i<count($dType);$i++){?>
				   <option value="<?php echo $dType[$i]['DocumentationType']['id'].'~'.$dType[$i]['DocumentationType']['type']; ?>" <?php if($d_type==$dType[$i]['DocumentationType']['id']){echo "selected";}else{} ?>><?php echo $dType[$i]['DocumentationType']['type']; ?></option>
				   <?php } ?>
			     </select>
<div class="clearflds"></div>		             
...
<h2 id='doc_type'><?php echo $doc_type; ?></h2>
<div class="clearflds"></div>
