...
function confirmFun(me)
    {
	  ...
	  if(confirm("Do You Want To Delete?"))
	  {
	       ...
	       var  uploadPath = rootpath+'Jhas/uploadimage/delete/'+encodeURIComponent(imagename);
...
function uploadFile()
     {
	  ...
	  var rootpath = '<?php echo $this->webroot;?>';
	  $('#loaderRes').html('<br /><img src='+rootpath+'/app/webroot/img/ajax-loader.gif>');
	  var  uploadPath = rootpath+'Jhas/uploadimage/upload';
	  document.add_report_attachment_form.action =uploadPath;
	  document.add_report_attachment_form.target = "imgFrame";
	  document.add_report_attachment_form.submit();
     }
...
function add_jha_attach()
{
	    ...
	    if(file_name==''){
	        document.getElementById('image_upload_res').innerHTML='<font color="red">Please upload file</font>';
	    ...
	    document.getElementById('loader').innerHTML='<img src="<?php echo $this->webroot; ?>img/loader.gif" />';	
	       ...
	       $.ajax({
			  type: "POST",
			  url: rootpath+"Jhas/jhaattachmentprocess/",
			...
			if(res=='fail'){
				   ...
				   document.location='<?php echo $this->webroot; ?>Jhas/report_jha_attachment_list/<?php echo base64_encode($report_id); ?>';
			     }else if(res=='update'){
...
<h2><?php echo $heading; ?> &nbsp; <?php echo $report_number; ?></h2>
  <?php
    echo $this->Form->create('Jhas', array('type'=>'file','id' => 'add_report_attachment_form','name'=>'add_report_attachment_form','class'=>'adminform'));
...
<div class="upload-here">
      <input type="file" size="24"  name="data[Jhas][file_upload]" id="BrowserHidden" onchange="getElementById('FileField').value = getElementById('BrowserHidden').value;" />
      <div id="BrowserVisible"><input type="text" id="FileField"  class="textclass" /></div><input type="button" name="save" id="save" class="buttonsave"  onclick="return uploadFile();" value="Upload" />
</div>
<div class="clearflds"></div>
<label>&nbsp;</label>
...
<span id="loader" style="font-size: 13px;"></span> <input type="button" name="save" id="save" class="buttonsave" onclick="add_jha_attach();" value="<?php echo $button; ?>" />
</div>
<?php echo $this->Form->end(); ?>
</section>