...
function add_report_main()
{
	    ...
	    var dataStr = $("#add_jha_main_form").serialize();
            var rootpath='<?php echo $this->webroot ?>';
     document.getElementById('loader').innerHTML='<img src="<?php echo $this->webroot; ?>img/loader.gif" />';	
              $.ajax({
			  type: "POST",
			  url: rootpath+"Jhas/jhareportprocess/",
			  data:"data="+dataStr+"&report_no="+report_no+"&business_unit="+business_unit+"&field_location="+field_location+"&country="+country+"&validate_by="+validate_by+"&report_id="+report_id+"&validation_date="+validation_date+"&revalidation_date="+revalidation_date+"&l_type="+sp_val[0],
			  ...
			  success: function(res)
			  {
			   ...
			   var resval=res.split("~");
			      if(resval[0]=='fail'){
				   ...
				   document.getElementById('loader').innerHTML='<font color="green">Job Hazard Analysis Report Added Successfully</font>';
				   document.location='<?php echo $this->webroot; ?>Jhas/add_jha_report_main/'+resval[1];
				   
                             }else if(resval[0]=='update'){
				   document.getElementById('loader').innerHTML='<font color="green">Job Hazard Analysis Repor Updated Successfully</font>';
				   document.location='<?php echo $this->webroot; ?>Jhas/add_jha_report_main/'+resval[1];
				  }
...
<label><?PHP echo __("Type:");?></label>
				   ...
				   <option value="<?php echo $lType[$i]['JhaType']['id'].'~'.$lType[$i]['JhaType']['type']; ?>" <?php if($l_type==$lType[$i]['JhaType']['id']){echo "selected";}else{} ?>><?php echo $lType[$i]['JhaType']['type']; ?></option>
				   <?php } ?>
			     </select>
<div class="clearflds"></div>		 
