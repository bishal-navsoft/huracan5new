...
function delete_incident(id){
	...
	$.ajax({
			  ...
			  {
			     if(res=='ok'){
				   document.location=path+'Sqreports/report_sq_incident_list/'+report_val;
				 }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Sqreports/incident_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Sqreports/incident_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Sqreports/get_all_sqincident_list/'+report_id}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"Sqreports/add_sq_incident/"+Base64.encode(records['data']['report_id'])+"/"+Base64.encode(records['data']['id']);
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Sqreports/incident_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Sqreports/incident_block/'+records['data']['id']+'/'
									 ,method:'GET'
   //,params:{id:record.data.id,con:'games',act:'movetoup'}
    ...
    var cm = new Ext.grid.ColumnModel([
	 checkBox,
        ...
        {header: incident_no,sortable: true, dataIndex: 'incident_no', width:105},
	{header: affected_service,sortable: true, dataIndex: 'sqservice_type', width:90},
	{header: incident_time,sortable: true, dataIndex: 'incident_time', width:110},
        {header: incident_date,sortable: true, dataIndex: 'incident_date', width:120},
        {header: incidentseverity_type,sortable: true, dataIndex: 'incidentseverity_type', width:130},
       	{header: loss_time,sortable: true, dataIndex: 'loss_time', width:130},
	{header: sqdamage_type,sortable: true, dataIndex: 'sqdamage_type', width:120},
    ...
    Ext.get('filterlabel').update(incident_time); 
