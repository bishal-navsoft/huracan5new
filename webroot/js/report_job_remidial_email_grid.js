...
function delete_remidial(id){
	  ...
	  {	
		$.ajax({
			  type: "POST",
			  url: path+"Jobs/remidial_email_delete/",
			  data:"id="+id,
			  success: function(res)
			  {
							
				if(res=='ok'){
				     document.location=path+'Jobs/job_remedila_email_list/'+report_val;
				   }
...
function view_remedial(id,remedial_no,report_id){
	jQuery.fancybox({
			...
			'transitionIn'		: 'fade',
			'transitionOut'		: 'fade',
			'href'			:  path+"Jobs/remidial_email_view/"+id+"/"+remedial_no+"/"+report_id,
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Jobs/get_all_remidial_email_list/'+report_id}),  //note that I used host in the url
