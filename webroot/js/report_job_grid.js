...
function delete_hsse_report_main(id){
     if(confirm("Do You Want To Delete?"))
	  {	
		$.ajax({
				  type: "POST",
				  url: path+"Jobs/job_report_delete/",
				  data:"id="+id,
				  success: function(res)
				  {
							
				     if(res=='ok'){
					   document.location=path+'Jobs/report_job_list';
					 }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Jobs/job_report_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Jobs/job_report_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Jobs/get_all_report/'+action+'/'}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"Jobs/add_job_report_main/"+Base64.encode(records['data']['id'])+"/";
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Jobs/job_report_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Jobs/job_report_block/'+records['data']['id']+'/'
									 ,method:'GET'
   //,params:{id:record.data.id,con:'games',act:'movetoup'}
			...
			callback:function(grid, records, action, groupId) {				
				...
				if(is_view == 1)
					{
					document.location=path+'Jobs/add_jobreport_view/'+Base64.encode(records['data']['id']);    
					}
					else
					{
						Ext.Msg.alert(warning,not_allowed_access);
    ...
    var cm = new Ext.grid.ColumnModel([
	 ...
	 Actions,
        ...
        {header: report_date,sortable: true, dataIndex: 'report_date_val', width:93},
	{header: comment,sortable: true, dataIndex: 'comment', width:240},
