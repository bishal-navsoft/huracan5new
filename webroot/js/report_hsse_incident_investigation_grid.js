...
function delete_incident_investigation(id){
	...
	$.ajax({
			  type: "POST",
			  url: path+"Reports/investigation_data_delete/",
			  data:"data=a&id="+id,
			  success: function(res)
			  {
				
		               if(res=='ok'){
				   document.location=path+'Reports/report_hsse_investigation_data_list/'+report_val;
				 }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Reports/investigation_data_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Reports/investigation_data_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Reports/get_all_hsse_investigation_data_list/'+report_id}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"Reports/add_investigation_data_analysis/"+Base64.encode(records['data']['report_id'])+"/"+Base64.encode(records['data']['incident_id'])+"/"+Base64.encode(records['data']['id']);
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Reports/investigation_data_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Reports/investigation_data_block/'+records['data']['id']+'/'
									 ,method:'GET'
   //,params:{id:record.data.id,con:'games',act:'movetoup'}
    ...
    var cm = new Ext.grid.ColumnModel([
	 checkBox,
        {header: investigation_no,sortable: true, dataIndex: 'investigation_no', width:70},
	{header: incident_no,sortable: true, dataIndex: 'incident_no', width:70},
	{header: lossname,sortable: true, dataIndex: 'incident_loss', width:70},
    ...
    Ext.get('filterlabel').update(lossname); 
