...
function delete_document_report_main(id){
       if(confirm("Do You Want To Delete?"))
	  {		
		$.ajax({
				  type: "POST",
				  url: path+"Documents/document_main_delete/",
				  data:"id="+id,
				  success: function(res)
				  {
							
				     if(res=='ok'){
					   document.location=path+'Documents/document_main_list';
					 }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Documents/document_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Documents/document_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
					       ...
					       else
					       {
						       ...
						       for(var i=0; i<selectedArray.length; i++)
						       {
							       ...
							       }else
							       {
				                      ...
				                      delete_document_report_main(selectedIds);
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Documents/get_all_report/'+action+'/'}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"Documents/add_document_report_main/"+Base64.encode(records['data']['id'])+"/";
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Documents/document_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Documents/document_block/'+records['data']['id']+'/'
									 ,method:'GET'
   //,params:{id:record.data.id,con:'games',act:'movetoup'}
			...
			callback:function(grid, records, action, groupId) {				
				...
				if(is_delete == 1)
					{
					  delete_document_report_main(records['data']['id']);
					}
					else
					{
						Ext.Msg.alert(warning,not_allowed_access);
			...
			callback:function(grid, records, action, groupId) {				
				...
				if(is_view == 1)
					{
					document.location=path+'Documents/add_dcreport_view/'+Base64.encode(records['data']['id']);    
					}
					else
					{
						Ext.Msg.alert(warning,not_allowed_access);
	...
	{header: type ,sortable: true, dataIndex: 'type', width:120},
    ...
    var grid = new Ext.grid.GridPanel({
	  ...
	  var filterMenuItems = [
	    ...
	    new Ext.menu.CheckItem({ 
		text: validate_date_val, 
		checked: true, 
		group: 'filter',
		id: 'validate_date_val'
		,checkHandler: onFilterItemCheck 
	    }),
	    new Ext.menu.CheckItem({ 
		text: re_revalidation_date, 
		checked: true, 
		group: 'filter',
		id: 'rvalidate_date_val'
		,checkHandler: onFilterItemCheck 
	    }),
	    new Ext.menu.CheckItem({ 
		text: create_date_val, 
		checked: true, 
		group: 'filter',
		id: 'create_date_val'
		,checkHandler: onFilterItemCheck 
