...
function delete_remidial(id){
   if(confirm("Do You Want To Delete?")){
		...
		$.ajax({
				  type: "POST",
				  url: path+"Suggestions/remidial_delete/",
				  data:"data=a&id="+id,
				  success: function(res)
				  {
					
				      if(res=='ok'){
					  document.location=path+'Suggestions/report_suggestion_remidial_list/'+report_val;
				    }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Suggestions/remidial_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Suggestions/remidial_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Suggestions/get_all_remidial_list/'+report_id}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"Suggestions/add_suggestion_remidial/"+Base64.encode(report_no)+"/"+Base64.encode(records['data']['id']);
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Suggestions/remidial_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Suggestions/remidial_block/'+records['data']['id']+'/'
									 ,method:'GET'
