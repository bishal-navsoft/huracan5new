...
function delete_remidial(id){
	  ...
	  {	
			$.ajax({
				  type: "POST",
				  url: path+"Suggestions/remidial_email_delete/",
				  data:"id="+id,
				  success: function(res)
				  {
								
					if(res=='ok'){
					     document.location=path+'Suggestions/report_suggestion_remedila_email_list/'+report_val;
					   }
...
function view_remedial(id,remedial_no,report_id){
	jQuery.fancybox({
			...
			'transitionIn'		: 'fade',
			'transitionOut'		: 'fade',
			'href'			:  path+"Suggestions/remidial_email_view/"+id+"/"+remedial_no+"/"+report_id,
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Suggestions/get_all_remidial_email_list/'+report_id}),  //note that I used host in the url
