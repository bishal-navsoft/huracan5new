...
function delete_sq_report_main(id){
	
	if(confirm("Do You Want To Delete?"))
	  {
		$.ajax({
				  type: "POST",
				  url: path+"Sqreports/sqreport_main_delete/",
				  data:"id="+id,
				  success: function(res)
				  {
							
				     if(res=='ok'){
					   document.location=path+'Sqreports/report_sq_list';
					 }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Sqreports/sqreport_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Sqreports/sqreport_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
					       ...
					       else
					       {
						       ...
						       for(var i=0; i<selectedArray.length; i++)
						       {
							       ...
							       }else
							       {
				                      ...
				                      delete_sq_report_main(selectedIds);
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Sqreports/get_all_report/'+action+'/'}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"Sqreports/add_sq_report_main/"+Base64.encode(records['data']['id'])+"/";
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Sqreports/sqreport_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Sqreports/sqreport_block/'+records['data']['id']+'/'
									 ,method:'GET'
   //,params:{id:record.data.id,con:'games',act:'movetoup'}
			...
			callback:function(grid, records, action, groupId) {				
				...
				if(is_delete == 1)
					{
					  delete_sq_report_main(records['data']['id']);
					}
					else
					{
						Ext.Msg.alert(warning,not_allowed_access);
			...
			callback:function(grid, records, action, groupId) {				
				...
				if(is_view == 1)
					{
					document.location=path+'Sqreports/add_sqreport_view/'+Base64.encode(records['data']['id']);    
					}
					else
					{
						Ext.Msg.alert(warning,not_allowed_access);
    ...
    var cm = new Ext.grid.ColumnModel([
	 ...
	 Actions,
        ...
        {header: event,sortable: true, dataIndex: 'event_date_val', width:93},
	{header: summary,sortable: true, dataIndex: 'summary', width:240},
       	{header: client,sortable: true, dataIndex: 'client_name', width:190},
	{header: operating_time,sortable: true, dataIndex: 'operating_time', width:130},
	{header: loss_time,sortable: true, dataIndex: 'loss_time', width:100},
	{header: incident_serverity,sortable: true, dataIndex: 'incident_serverity', width:150},
