...
//Function for unblock selected records
	function delete_remidial(id)
	{
		if(confirm("Do You Want To Delete?"))
		{
			$.ajax({
				type: "POST",
				url: path+"Reports/remidial_delete/",
				data: { id: id },
				headers: {
				...
				{
					if(res=='ok'){
						document.location=path+'Reports/report_hsse_remidial_list/'+report_val;
					}
				}
			});
		}else{
	...
	var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Reports/get_all_remidial_list/'+report_id}),  //note that I used host in the url
		...
		,actions: [{
			...
			callback:function(grid, records, action, groupId) {	
				if(is_edit == 1)
				{
					...
					location.href = path+"Reports/add_hsse_remidial/"+Base64.encode(String(report_no))+"/"+Base64.encode(String(records['data']['id']));
				}
				else
				{
					Ext.Msg.alert(warning,not_allowed_access);
						...
						Ext.Msg.show({
							...
							,fn:function(response) {
								...
								else
								{
									...
									Ext.Ajax.request(
									{
										url: path+'Reports/remidial_unblock/'+records['data']['id']+'/'
										,method:'GET'
										,success: function(response){
											ds.reload();
											box.hide();
					...
					if(is_block == 1)
					{
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Reports/remidial_block/'+records['data']['id']+'/'
