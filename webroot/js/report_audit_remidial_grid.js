...
function delete_remidial(id){
	...
	$.ajax({
			  type: "POST",
			  url: path+"Audits/remidial_delete/",
			  data:"data=a&id="+id,
			  success: function(res)
			  {
			     	
                              if(res=='ok'){
				  document.location=path+'Audits/report_audit_remidial_list/'+report_val;
			    }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Audits/remidial_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Audits/remidial_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Audits/get_all_remidial_list/'+report_id}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"Audits/add_audit_remidial/"+Base64.encode(report_no)+"/"+Base64.encode(records['data']['id']);
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Audits/remidial_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Audits/remidial_block/'+records['data']['id']+'/'
									 ,method:'GET'
