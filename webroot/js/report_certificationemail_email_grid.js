...
function delete_cetification_email(id){

		$.ajax({
			  type: "POST",
			  url: path+"Certifications/cetification_email_delete/",
			  data:"id="+id,
			  success: function(res)
			  {
				if(res=='ok'){
				     document.location=path+'Certifications/cerficate_email_list';
				   }
	                    }
	});
					...
					title:del_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       if(is_delete == 1)
						         {
						          delete_cetification_email(selectedIds);
						        }
						        else
							{
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Certifications/get_all_certificationemail_list'}),  //note that I used host in the url
	...
	//alert(eval(pagelmt));
		...
		var Actions = new Ext.ux.grid.RowActions({
			...
			callback:function(grid, records, action, groupId) {				
				...
				if(is_delete == 1)
					{
					  delete_cetification_email(records['data']['id']);
					}
					else
					{
						Ext.Msg.alert(warning,not_allowed_access);
	...
	{header: cert_type,sortable: true, dataIndex: 'cert_type', width:240},
	{header: crt_date,sortable: true, dataIndex: 'crt_date', width:120},
        {header: email_date,sortable: true, dataIndex: 'email_date', width:105},
	{header: email,sortable: true, dataIndex: 'email', width:190},
	{header: send_person,sortable: true, dataIndex: 'send_person', width:200}, 
	{header: status_value,sortable: true, dataIndex: 'status_value', width:140},
    	Actions
