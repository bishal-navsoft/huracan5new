...
function delete_remidial(id){
		...
		$.ajax({
			  type: "POST",
			  url: path+"Documents/remidial_email_delete/",
			  data:"id="+id,
			  success: function(res)
			  {
							
				if(res=='ok'){
				     document.location=path+'Documents/document_email_list/'+report_val;
				   }
...
function view_remedial(id,leid){
	jQuery.fancybox({
			...
			'transitionIn'		: 'fade',
			'transitionOut'		: 'fade',
			'href'			:  path+"Documents/document_email_view/"+id+"/"+leid,
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Documents/get_all_document_email_list/'+report_id}),  //note that I used host in the url
