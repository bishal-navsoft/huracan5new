...
function delete_link(id){
	...
	$.ajax({
			  type: "POST",
			  url: path+"Lessons/link_delete/",
			  data:"data=a&id="+id,
			  success: function(res)
			  {
				
		               if(res=='ok'){
				   document.location=path+'Lessons/report_lesson_link_list/'+report_id+'/'+Base64.encode(filterTYPE);
				 }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Lessons/link_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Lessons/link_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Lessons/get_all_link_list/'+report_id+'/'+filterTYPE}),  //note that I used host in the url
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Lessons/link_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Lessons/link_block/'+records['data']['id']+'/'
									 ,method:'GET'
