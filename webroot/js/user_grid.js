...
function delete_user(id){
	...
	$.ajax({
			  type: "POST",
			  url: path+"AdminMasters/user_delete/",
			  data:"id="+id,
			  success: function(res)
			  {
		             if(res=='ok'){
				    document.location=path+'AdminMasters/user_list';     
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'AdminMasters/user_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'AdminMasters/user_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'AdminMasters/get_all_user/'+action+'/'}),  //note that I used host in the url
		...
		var Actions = new Ext.ux.grid.RowActions({
			...
			callback:function(grid, records, action, groupId) {	
				if(is_edit == 1)
				{
					...
					location.href = path+"AdminMasters/add_user/"+Base64.encode(records['data']['id'])+"/";
				}
				else
				{
					Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'AdminMasters/user_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'AdminMasters/user_block/'+records['data']['id']+'/'
									 ,method:'GET'
									 /*,params: {
										id:        record.data.id,
										con:    'games',
    ...
    var cm = new Ext.grid.ColumnModel([
		checkBox,
                {header: fullname,sortable: true, dataIndex: 'name', width: 170},
	        {header: uname,sortable: true, dataIndex: 'admin_user', width: 130},
		{header: position,sortable: true, dataIndex: 'roll_name', width: 130},
	        {header: email,sortable: true, dataIndex: 'admin_email', width: 220},
	        {header: phone,sortable: true, dataIndex: 'phone', width: 100},
	        //{header: position,sortable: true, dataIndex: 'position', width: 100},
	        {header: seniority,sortable: true, dataIndex: 'seniority', width: 100},
	        {header: position,sortable: true, dataIndex: 'position', width: 100},
