...
function delete_gauge(id){
	$.ajax({
			  type: "POST",
			  url: path+"Jobs/gaugedata_delete/",
			  data:"id="+id,
			  success: function(res)
			  {
						
		             if(res=='ok'){
				   document.location=path+'Jobs/report_gauge_job_list/'+Base64.encode(records['data']['report_id']);
				 }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Jobs/gaugedata_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Jobs/gaugedata_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Jobs/get_all_gauge_list/'+report_id}),  //note that I used host in the url
	...
	{name: 'press_range'},
        {name: 'temp_range'},
	{name: 'manufacture'},
        {name: 'gauge_set_depth'},
	{name: 'tec_cable'},
	{name: 'ysplitre'},
	{name: 'who_connector'},
	{name: 'sau'},
	{name: 'gauge_type_val'},
	{name: 'tec_cable_value'},
	{name: 'temp_range_value'},
	{name: 'ysplitter_value'},
	{name: 'press_range_value'},
	{name: 'sau_value'},
	{name: 'manufacture_value'},
	{name: 'gauge_sn'},
	{name: 'isblocked'},
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							document.location=path+"Jobs/add_gauge_job_data/"+Base64.encode(records['data']['report_id'])+"/"+Base64.encode(records['data']['id']);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Jobs/gaugedata_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Jobs/gaugedata_block/'+records['data']['id']+'/'
									 ,method:'GET'
   //,params:{id:record.data.id,con:'games',act:'movetoup'}
    ...
    var cm = new Ext.grid.ColumnModel([
	 checkBox,
        ...
        {header: gauge_no,sortable: true, dataIndex: 'gauge_no', width:80},
        {header: gauge_sn,sortable: true, dataIndex: 'gauge_sn', width:110},
	{header: gauge_type,sortable: true, dataIndex: 'gauge_type_val', width:110},
	{header: press_range,sortable: true, dataIndex: 'press_range_value', width:110},
	{header: temp_range,sortable: true, dataIndex: 'temp_range_value', width:120},
	{header: manufacture,sortable: true, dataIndex: 'manufacture_value', width:100},
        {header: gauge_set_depth,sortable: true, dataIndex: 'gauge_set_depth', width:100},
        {header: ysplitter,sortable: true, dataIndex: 'ysplitter_value', width:100},
	{header: ysplitter,sortable: true, dataIndex: 'tec_cable_value', width:100},
	{header: sau,sortable: true, dataIndex: 'sau_value', width:100},
    ...
    Ext.get('filterlabel').update(gauge_type); 
