...
function delete_jn_report_main(id){
	$.ajax({
			  type: "POST",
			  url: path+"Jrns/jnreport_main_delete/",
			  data:"id="+id,
			  success: function(res)
			  {
						
		             if(res=='ok'){
				   document.location=path+'Jrns/report_jrn_list';
				 }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Jrns/jnreports_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Jrns/jnreport_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
					       ...
					       else
					       {
						       ...
						       for(var i=0; i<selectedArray.length; i++)
						       {
							       ...
							       }else
							       {
				                      ...
				                      delete_jn_report_main(selectedIds);
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Jrns/get_all_report/'+action+'/'}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"Jrns/add_jn_report_main/"+Base64.encode(records['data']['id'])+"/";
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
								 	url: path+'Jrns/jnreport_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Jrns/jnreport_block/'+records['data']['id']+'/'
									 ,method:'GET'
   //,params:{id:record.data.id,con:'games',act:'movetoup'}
			...
			callback:function(grid, records, action, groupId) {				
				...
				if(is_delete == 1)
					{
					  delete_jn_report_main(records['data']['id']);
					}
					else
					{
						Ext.Msg.alert(warning,not_allowed_access);
			...
			callback:function(grid, records, action, groupId) {				
				...
				if(is_view == 1)
					{
					document.location=path+'Jrns/add_jrnreport_view/'+Base64.encode(records['data']['id']);    
					}
					else
					{
						Ext.Msg.alert(warning,not_allowed_access);
    ...
    var cm = new Ext.grid.ColumnModel([
	 ...
	 Actions,
        ...
        {header: trip_number,sortable: true, dataIndex: 'trip_number', width:135},
        {header: trip_date,sortable: true, dataIndex: 'trip_date_val', width:93},
	{header: summary,sortable: true, dataIndex: 'summary', width:240},
       	{header: client,sortable: true, dataIndex: 'client_name', width:190},
	{header: departure_time,sortable: true, dataIndex: 'departure_time', width:130},
