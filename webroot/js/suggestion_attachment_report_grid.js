...
function delete_attachment(id){
	...
	$.ajax({
			  type: "POST",
			  url: path+"Suggestions/attachment_delete/",
			  data:"data=a&id="+id,
			  success: function(res)
			  {
		
			   if(res=='ok'){
				document.location=path+'Suggestions/report_suggestion_attachment_list/'+root_param;
			   }
				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'Suggestions/attachment_unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'Suggestions/attachment_block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'Suggestions/get_all_attachment_list/'+report_id}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"Suggestions/add_suggestion_attachment/"+Base64.encode(records['data']['report_id'])+"/"+Base64.encode(records['data']['id']);
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'Suggestions/attachment_unblock/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									 url: path+'Suggestions/attachment_block/'+records['data']['id']+'/'
									 ,method:'GET'
   //,params:{id:record.data.id,con:'games',act:'movetoup'}
    ...
    var cm = new Ext.grid.ColumnModel([
	 checkBox,
        ...
        {header: filename,sortable: true, dataIndex: 'image_src', width:115},
        {header: description,sortable: true, dataIndex: 'description', width:450},
	{header: stts, sortable: true, renderer: status, dataIndex: 'isblocked', width:100},
	Actions
