				...
				title:activ_select_record
			       ...
			       ,fn:function(response) {
				       ...
				       else
				       {
					       ...
					       Ext.Ajax.request(
										       {
													url: path+'RoleMasters/unblock/'+selectedIds+'/'
													,method:'GET'
					...
					title:deactiv_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'RoleMasters/block/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
					...
					title:del_select_record
				       ...
				       ,fn:function(response) {
					       ...
					       else
					       {
						       ...
						       Ext.Ajax.request(
											       {
														url: path+'RoleMasters/delete/'+selectedIds+'/'
														,method:'GET'
														,success: function(response){
														       ds.reload();
														       box.hide();
...
var ds = new Ext.data.Store({	
        proxy: new Ext.data.HttpProxy({url: AdminListPage+'RoleMasters/get_all_admin_role/'+action+'/'}),  //note that I used host in the url
				...
				,actions: [{
					...
					callback:function(grid, records, action, groupId) {	
						if(is_edit == 1)
						{
							...
							location.href = path+"RoleMasters/add_roll/"+Base64.encode(records['data']['id'])+"/";
						}
						else
						{
							Ext.Msg.alert(warning,not_allowed_access);
					...
					Ext.Msg.show({
						...
						,fn:function(response) {
							...
							else
							{
								...
								Ext.Ajax.request(
								{
									url: path+'RoleMasters/' + turl + '/'+records['data']['id']+'/'
									,method:'GET'
									,success: function(response){
										ds.reload();
										box.hide();
						   ...
						   ,fn:function(response) {
								...
								else
								{
									...
									Ext.Ajax.request({
										...
										,url: path+'RoleMasters/delete/'+records['data']['id']+'/'
										,method:'GET'
																		/*,params: {
											   id:        record.data.id,
											   con:    'games',
    ...
    var cm = new Ext.grid.ColumnModel([
        ...
        /*{header: "ID", dataIndex: 'id', width: 100, hidden: true},*/
				{header: role,sortable: true, dataIndex: 'role_name', width: 260},
        //{header: "Email", dataIndex: 'email', width: 200},
        {header: description,sortable: true, dataIndex: 'description', width: 410},
		
        {header: stts, sortable: true, renderer: status, dataIndex: 'isblocked', width: 155},
		Actions
